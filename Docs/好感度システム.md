...
3. **セリフの分岐** — 好感度の値に応じて次のセリフを切り替えます。
...
1. `branches` を先頭から評価し、最初に条件を満たした分岐先へ移動
2. 分岐が無い・一致しない場合は `nextLineId` へ移動
3. `nextLineId` も空の場合は CSV 上の次の行へ移動

---

## 5. CSV フォーマット

ダイアログ CSV に以下の 2 列を追加できます（どちらも省略可能）。

```
lineId,speakerId,expressionId,text,nextLineId,affectionChanges,branches
```

### 5.1 affectionChanges（好感度の増減）
...
prologue_002,kayo,normal,"「でも、食べすぎてちょっと太ったかも」",prologue_003,,
prologue_003,doute,normal,"「今何kgなの？」",prologue_004,doute+2,kayo>=5->prologue_004_good; kayo<5->prologue_004_bad
prologue_004_good,kayo,normal,「……ばかっ！聞かないでよ」,,,
prologue_004_bad,kayo,normal,「なんで聞くのよ、もう帰る」,,,
```
